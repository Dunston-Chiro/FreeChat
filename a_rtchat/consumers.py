...
class ChatroomConsumer(WebsocketConsumer):
    ...
    #             'message': {
    #                 'level': 'warning',
    #                 'text': 'You have been removed from the chat.',
    #             },
    #             'url': '/',
...
class OnlineStatusConsumer(WebsocketConsumer):
    ...
    def online_status_handler(self, event):
        ...
        group_chat_with_users = [chat for chat in my_chats.filter(groupchat_name__isnull=False) if chat.users_online.exclude(id=self.user.id)]

        if public_chat_users or private_chats_with_users or group_chat_with_users:
        ...
        html = render_to_string("a_rtchat/partials/onlne_status.html", context=context)
        self.send(text_data=html)